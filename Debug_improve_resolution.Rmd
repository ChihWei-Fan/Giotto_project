...
input_mat[,1]<- sapply(input_mat$tile_name,basename) #input_mat include tile_name, 2040 features, APOE, and MMP7 expression
...
mse =  mean((test_set$APOE - APOE_preds)^2) 
mae = MAE(test_set$APOE, APOE_preds)
rmse = RMSE(test_set$APOE, APOE_preds)
r2 = R2(test_set$APOE, APOE_preds, form = "corr")
cat(" MAE:", mae, "\n", "MSE:", mse, "\n", 
    "RMSE:", rmse, "\n", "R-squared:", r2)

plot(test_set$APOE, APOE_preds)
scatter.smooth(test_set$APOE, APOE_preds)
cor(test_set$APOE, APOE_preds)
...
# Make predictions on the test set
APOE_preds <- predict(APOE_glmnet, newdata = test_set[,-c(1, 2050, 2051)], type = "raw")

#Generate confusion matrix
cm_APOE = data.frame( APOE = test_set[2050], APOE_pred = APOE_preds, tile_name = test_set[,1])
...
ggplot(cm_APOE, aes(x=x_cor, y=y_cor)) +
  geom_point(aes(colour = APOE_preds)) +
...
APOE_rf <- randomForest(APOE ~ ., data = training_set[, -c(1, 2051)], importance = TRUE)

# Make predictions on the test set
APOE_rf_preds <- predict(APOE_rf, newdata = test_set[,-c(1, 2050, 2051)], type = "response")


#Generate confusion matrix
cm_APOE_rf = data.frame( APOE = test_set[2050], APOE_pred = APOE_rf_preds, tile_name = test_set[,1])

# Combine the original expression value and predict values with tile coordinates
cm_APOE_rf <- merge(tile_plot_df[match(cm_APOE_rf$tile_name, tile_plot_df$tile_name),c(3,9,13,14)], cm_APOE_rf, by = "tile_name")

mse =  mean((test_set$APOE - APOE_rf_preds)^2) 
mae = MAE(test_set$APOE, APOE_rf_preds)
rmse = RMSE(test_set$APOE, APOE_rf_preds)
r2 = R2(test_set$APOE, APOE_rf_preds, form = "corr") #change the R-square function
cat(" MAE:", mae, "\n", "MSE:", mse, "\n", 
    "RMSE:", rmse, "\n", "R-squared:", r2)

#Polt predict expression
ggplot(cm_APOE_rf, aes(x=x_cor, y=y_cor)) +
  geom_point(aes(colour = APOE_rf_preds)) +
  scale_colour_gradient2()
...
APOE_gbm <-gbm(APOE~., 
               ...
               distribution = "gaussian",   # Distribution of the response variable. "gaussian" for regression.
               cv.folds = 10,     
               shrinkage = 0.01,  # Shrinkage parameter (learning rate) to control the contribution of each tree.
               n.minobsinnode = 10,  # Minimum number of observations in a terminal node of a tree.
               n.trees = 500
               ) 

# Make predictions on the test set
APOE_gbm_preds <- predict(APOE_gbm, newdata = test_set[,-c(1, 2050, 2051)], type = "response")


#Generate confusion matrix
cm_APOE_gbm = data.frame( APOE = test_set[2050], APOE_pred = APOE_gbm_preds, tile_name = test_set[,1])

# Combine the original expression value and predict values with tile coordinates
cm_APOE_gbm <- merge(tile_plot_df[match(cm_APOE_gbm$tile_name, tile_plot_df$tile_name),c(3,9,13,14)], cm_APOE_gbm, by = "tile_name")

mse =  mean((test_set$APOE - APOE_gbm_preds)^2) 
mae = MAE(test_set$APOE, APOE_gbm_preds)
rmse = RMSE(test_set$APOE, APOE_gbm_preds)
r2 = R2(test_set$APOE, APOE_gbm_preds, form = "corr")
cat(" MAE:", mae, "\n", "MSE:", mse, "\n", 
    "RMSE:", rmse, "\n", "R-squared:", r2)

#Polt predict expression
ggplot(cm_APOE_gbm, aes(x=x_cor, y=y_cor)) +
  geom_point(aes(colour = APOE_gbm_preds)) +
  scale_colour_gradient2()
```
#Model4:  SVM
```{r}
...
svm_mod_APOE <- svm(APOE ~ .,
                    data = training_set[,-c(1,2051)], #c(1,11)
                    type = 'eps-regression',
                    kernel = 'polynomial',
                    cost = 30)
...
APOE_svm_preds = predict(svm_mod_APOE , newdata = test_set[,-c(1,2050, 2051)])

#Generate confusion matrix
cm_APOE_svm = data.frame( APOE = test_set[2050], APOE_pred = APOE_svm_preds, tile_name = test_set[,1]) #APOE = test_set[10]


# Combine the original expression value and predict values with tile coordinates
cm_APOE_svm <- merge(tile_plot_df[match(cm_APOE_svm$tile_name, tile_plot_df$tile_name),c(3,9,13,14)], cm_APOE_svm, by = "tile_name")


mse =  mean((test_set$APOE - APOE_svm_preds)^2) 
mae = MAE(test_set$APOE, APOE_svm_preds)
rmse = RMSE(test_set$APOE, APOE_svm_preds)
r2 = R2(test_set$APOE, APOE_svm_preds, form = "corr")
cat(" MAE:", mae, "\n", "MSE:", mse, "\n", 
    "RMSE:", rmse, "\n", "R-squared:", r2)

#Polt predict expression
ggplot(cm_APOE_svm, aes(x=x_cor, y=y_cor)) +
  geom_point(aes(colour = APOE_svm_preds)) +
  scale_colour_gradient2()
```
