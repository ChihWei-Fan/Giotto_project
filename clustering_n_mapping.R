...
#Load the workspace
load(file = "/projectnb/rd-spat/HOME/ivycwf/project_1/resolution/s119B_88x93_tiles/s119B_afterPCA.RData")

# Check if I remove and fill all the NaN values 
any(is.nan(image_mat))
dim(image_mat) # 4424 2048
...
explained_variance <- data.frame(PC= paste0("PC",1:2048), #used be 968
                                 var_explained= s119B_pca$sdev^2/sum((s119B_pca$sdev)^2))
...
pdf("/projectnb/rd-spat/HOME/ivycwf/project_1/resolution/s119B_88x93_tiles/s119B_scree_plot_pc15.pdf")
scree_plot_pc15
dev.off()
...
subset_pca_matrix <- pca_matrix[,1:8]            #centers =5, nstart = 10

# Decide how many clusters you want #
k_res8 = stats::kmeans(subset_pca_matrix, centers = 8, iter.max = 500, nstart = 100)
...
df <- data.frame(PC1 = pca_matrix[, 1], PC2 = pca_matrix[, 2], cluster = as.factor(k_res8$cluster))
...
pdf("/projectnb/rd-spat/HOME/ivycwf/project_1/resolution/s119B_88x93_tiles/s119B_pca8_cen8_it500_star100.pdf")
...
pca_res = data.table::data.table(ID = unlist(lapply(xempty_ls, basename)),
                                 PC1 = pca_matrix[,1], PC2 = pca_matrix[,2], kmeans = k_res8$cluster)
...
for (file_i in 1:length(pca_res$ID)) {
  ...
  x <- ifelse((numeric_part %% 88) == 0, 88, (numeric_part %% 88))
  y <- (numeric_part - 1) %/% 88 + 1
...
pl
pdf("/projectnb/rd-spat/HOME/ivycwf/project_1/resolution/s119B_88x93_tiles/s119B_pca8_cen8_it500_star100_result_small_font.pdf", width=11.2, height=10)
pl
dev.off()


#save.image(file = "/projectnb/rd-spat/HOME/ivycwf/project_1/resolution/s119B_88x93_tiles/s119B_pca8_kmean8.RData")
